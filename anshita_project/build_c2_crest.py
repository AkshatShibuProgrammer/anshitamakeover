...
with open('core/static/core/images/brand/sinha_c5_faces_pure.svg', 'r', encoding='utf-8') as f:
    face_raw = f.read()

# Extract inner groups from face_svg
inner_g = face_raw[face_raw.find('<g fill="currentColor">'):face_raw.rfind('</svg>')]
...
for side in [-1, 1]:
    for i in range(13):
        t = (i + 1) / 14.0
        # angle from bottom (math.pi/2) spreading upwards
...
# Crown paths (Imperial crown atop medallion, center x=500, base y=235)
crown_svg = '''  <!-- IMPERIAL ROYAL CROWN (CONCEPT 02 STYLE) -->
  <g id="imperial-crown">
    ...
    <!-- Gemstone Cutouts on Circlet Band -->
...
# Scale & center Concept 05 faces:
# Face SVG viewBox: 0 0 628 550.
# Center: cx=500, cy=510. Scale=0.64.
# Width = 628*0.64 = 401.92. Height = 550*0.64 = 352.
# x = 500 - 401.92/2 = 299. y = 510 - 352/2 = 334.
faces_placed = f'''  <!-- EMBEDDED AUTHENTIC CONCEPT 05 VECTOR FACES -->
  <g id="sinha-portrait-faces" transform="translate(299, 334) scale(0.64)">
{inner_g}
  </g>'''

# Straight, clean brand typography:
# User requirement: "dont put est year or sinha at below curve"
...
print('SUCCESS: Generated sinha_royal_c2_crest.svg! Size:', len(full_svg))
